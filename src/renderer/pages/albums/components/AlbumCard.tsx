...
/**
 * Classes for the artwork button. Every state lights the border up with a
 ...
 * border so it reads as a thicker, stronger lamp; the expanded card and the
 * card of the playing / paused album keep only the plain border plus the
 * blur, so they stay visible once the pointer leaves without competing with
 * the hovered card.
 */
const artworkClassName = (lit: boolean): string =>
  cn(
 ...
 * play overlay, then name / artist / year. Hovering the card, expanding it,
 * and playing / pausing one of its tracks light the artwork border up with a
 * glow (see `artworkClassName`); a stopped player lights nothing.
    ...
    <div className="group shrink-0" style={{ width }}>
      <div className="relative">
        <button
          ...
          className={artworkClassName(expanded || playing)}
          onClick={onToggle}
        >
