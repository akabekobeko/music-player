...
- `db.prepare(...).get()` / `.all()` の戻り値を `as` でキャストしない。zod スキーマの `parse` に通してから返す
  - SQLite の列型は親和性にすぎず、型と食い違う値が格納され得るため ([データベース](../specs/v1.0/architecture/database.md) の「クエリ結果の検証」)
- ドメイン型 (`Music` など) のスキーマは `src/shared/schemas/` に置き、`src/main/ipc/types.ts` の型は `z.infer` で導出する (スキーマが唯一の定義、手書きの型を並べない)
- クエリ固有の行 (`SELECT id ...`、集計行) のスキーマは、そのクエリのファイルにローカル定数として定義する。複数のクエリで共有する行は独立ファイルへ切り出す (例: `db/idRowSchema.ts`)
- スキーマの制約は型契約に留め、値域などの業務ルールを読み出し側で追加しない (正常なデータを読めなくするため)
