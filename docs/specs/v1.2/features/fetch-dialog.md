...
- 「失敗」の各行は `IpcError.code` ごとの文言 (`MB_NETWORK` / `MB_TIMEOUT` / `MB_THROTTLED` は [曲情報ダイアログの取得ボタン](music-info-fetch.md) と同じ `musicBrainzErrorKeyOf` の文、それ以外はメッセージ) を出します。失敗した曲がすべて同じ原因 (接続不可 / タイムアウト / 混雑) なら、一覧の上に案内を 1 行だけ出し、各行はファイル名だけにします。再試行は自動では行いません
