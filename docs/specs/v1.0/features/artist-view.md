...
- 固定ヘッダー: 検索ボックスの右隣に Lucide `TextInitial` のアイコンのみボタン (長めの遅延でツールチップ「頭文字へ移動」)。クリックでポップオーバーを開き、A–Z + その他のタイルを 7 列 × 4 行 (4 行目の Z の後にその他を 2 セル幅で配置) で表示。タイルは hover で枠線がぼかし付きで発光する。アーティストのないイニシャルは無効化し、クリックで該当セクションの見出しがリスト上端に来るまでスクロールする
