...
- **Shuffle**: 同じ全曲をシャッフルした順でキューに設定して再生 (シャッフルは「キュー生成時に並びを混ぜる」方式。PlayerBar のシャッフルモードを ON にし、モード ON 中はどのビューからの再生もシャッフルされる)
- **アルバムのインデックス ジャンプ**: Shuffle の左に Lucide `LayoutGrid` の円形アイコンボタン (ツールチップ「アルバムへ移動」)。クリックでポップオーバーを開き、表示中アルバムのアートワークを角丸の小さなタイル (40px 角、アートワークのないアルバムは `Disc3` のプレースホルダー) として 1 行 5 列のグリッドで表示する (`AlbumIndexPicker`)。タイルは hover で枠線がぼかし付きで発光し、遅延付きツールチップでアルバム名を表示する。曲絞り込みで非表示になったアルバムはグリッドにも出ない。クリックでポップオーバーを閉じ、該当アルバムの見出し行がリスト上端に来るまでスクロールする。アルバムがなければボタンは無効
- メニュー ([⋯]): Play artist / Shuffle play / Add to playlist (メニュー文言は再生対象を明示する: アーティストは「アーティストを再生」、アルバムは「アルバムを再生」、曲は「曲を再生」。シャッフルは選択で再生も始まるため「シャッフル再生」) (audio-player では未実装だった `onSelect` を必ず実装する)
