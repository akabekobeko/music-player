...
- 固定ヘッダー: 検索ボックスの右隣に Lucide `TextInitial` のアイコンのみボタン (長めの遅延でツールチップ「頭文字へ移動」)。クリックでポップオーバーを開き、A–Z + その他のタイルを 9 列 × 3 行 (3 行目の Z の後がその他) で表示。アーティストのないイニシャルは無効化し、クリックで該当セクションの見出しがリスト上端に来るまでスクロールする
