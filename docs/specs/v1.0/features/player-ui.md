...
PlayerBar はウィンドウ最上段・全幅の帯で、タイトルバー相当のドラッグ領域を兼ねます。ウィンドウ操作コントロールの配置とセーフエリアの前提は [ルーティングとレイアウト](../renderer/routing-layout.md) を参照。

```
┌──────────────────────────────────────────────────────────────┐
│ (余白)                                                (余白)  │ ← セーフエリア (drag)
│              [Art] Title / Artist — Album                    │
│ [⏮][⏯][⏭][⏹]   0:42 ─────────●───────── 3:28         [🔊]  │ ← 左右コントロールは下寄せ
└──────────────────────────────────────────────────────────────┘
```

- 中央ブロック (アートワーク + 曲情報 + SeekBar) は帯の高さを最大に使います
- コントロールは左右へ振り分け、**領域下寄せ**で配置します
  - 左下: 前曲・再生/一時停止・次曲・停止
  - 右下: 音量、キュー ([Playlist](playlist.md) のキュー Popover)
  - 帯の上端はドラッグ用の余白として空けます。帯の高さが十分あれば、ウィンドウ幅を狭めて左右端のコントロールがセーフエリアに近づいても、上端のウィンドウ操作コントロールとは垂直方向で住み分けられ、衝突しません
- 背景と余白は drag、ボタン・SeekBar・Popover などの対話要素は no-drag です
...
- snapshot の `error` を PlayerBar 直下のインラインアラート (shadcn Alert / destructive) で表示します。閉じるまで残り、曲切替で自動クリアされます (PlayerBar は最上段のため、上側はウィンドウ操作コントロールと干渉します)
