...
**コンテンツ領域側** — 右端に、ビューごとの曲絞り込みテキストボックスを置きます ([各ビュー仕様](../features/))。右の余白は `--content-toolbar-inset-right` で、macOS ではコンテンツ ヘッダーの右余白 (`pr-8`) と同じ値にしてテキストボックスの右端をヘッダーのメニュー ボタンと揃え、Windows / Linux では WCO セーフエリア + 1 文字分 (`1rem`) の余白を取ります。
